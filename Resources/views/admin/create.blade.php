      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary">
          ...
          <!-- /.box-header -->
					...
					<!-- form start -->
					{!! Form::open(['route' => 'admin.store', 'role'=>'form', 'autocomplete'=>'off', 'id'=>'formAdmin']) !!}	
						<div class="box-body">
							...
							<div class="form-group {{ ($errors->first('name')) ? 'has-error' : '' }}">
								...
								<input type="text" class="form-control" id="fname" placeholder="Name" name="name" value="{{ old('name') }}" required data-parsley-trigger="keyup focusout">
								@if($errors->has('name'))										
							...
							<div class="form-group {{ ($errors->first('role')) ? 'has-error' : '' }}">
								...
								<select name="role" class="form-control" id="frole" required data-parsley-trigger="keyup focusout">
									...
									@foreach($roles as $role)
										<option data-slug="{{ $role->slug }}" value="{{ $role->id }}" {{ (old('role')==$role->id) ? 'selected' : '' }}>{{ $role->name }}</option>
									@endforeach
								</select>
								@if($errors->has('role'))										
							...
							<div class="form-group {{ ($errors->first('address')) ? 'has-error' : '' }}">
								<label for="fphone">Address {{ old('role') }}</label>
